...
class QuickOptimizer:
    ...
    def __init__(self, population_csv_path: str, bus_stops_shp_path: str, max_stations: int = 100):
        ...
        logger.info(f"⚡ 轻量级模式：只使用前 {len(self.bus_stops_data)} 个站点进行测试")
        
        # 初始化奖励函数（提高覆盖率权重，让微小提升也有价值）
        self.reward_function = RewardFunction(
            self.population_data, 
            self.bus_stops_data, 
            coverage_radius=0.01,
            coverage_weight=100.0,  # 提高100倍，让0.01%的提升也能抵消移动惩罚
...
def test_all_modules_quickly():
    ...
    try:
        ...
        logger.info(f"✓ 数据加载成功: {len(pop_data)} 人口点, {len(bus_data)} 站点（测试版）")
        ...
        logger.info("\n=== 所有模块测试通过! ===")
        logger.info("✨ 轻量级测试完成，系统运行正常")
        logger.info("💡 如需完整测试，请运行 quick_test.py")
